...
function optimizeCrisisPlan({ cities, incidents, people = 1000, objective = "balanced", stress = "normal" }) {
  const startedAt = performance.now();
  const selectedIncidents = selectIncidents(incidents, people, objective, stress);
  const graph = buildRescueGraph(cities, selectedIncidents, { stress });
  ...
  const allocationDetails = flow.allocations
    ...
    .sort((a, b) => a.riskWeight - b.riskWeight || a.travelMinutes - b.travelMinutes || b.people - a.people);
  ...
  const averageArrival = allocationDetails.length
    ...
    : 0;
  const averageRiskWeight = allocationDetails.length
    ? Math.round(allocationDetails.reduce((sum, item) => sum + item.riskWeight, 0) / allocationDetails.length)
    : 0;

  return {
    objective,
    stress,
    stressLabel: graph.stressProfile.label,
...
function selectIncidents(incidents, people, objective, stress) {
  const ranked = [...incidents].sort((a, b) => {
    const scoreA = scoreIncident(a, objective, stress);
    const scoreB = scoreIncident(b, objective, stress);
...
function scoreIncident(incident, objective, stress) {
  const base = incident.people * 0.62 + incident.severity * 18;
  const stressBoost = stress === "blocked" ? incident.severity * 7 : stress === "congested" ? incident.severity * 3 : 0;
  if (objective === "coverage") return base + incident.people * 0.28;
  if (objective === "fastest") return base - incident.ageMinutes * 0.9;
  return base + incident.severity * 6 + stressBoost;
}
