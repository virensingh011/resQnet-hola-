...
function buildRescueGraph(cities, incidents) {
  ...
  for (const hub of hubs) {
    for (const incident of incidents) {
      ...
      const travelMinutes = Math.round(8 + (distanceKm / speedKmh) * 60 * hazardPenalty);
      const capacity = Math.max(30, Math.round(hub.capacity * Math.max(0.16, 1 - distanceKm / 2600) * (1 - incident.severity / 320)));
      const edge = { from: hub.id, to: incident.id, weight: travelMinutes, capacity, distanceKm: Math.round(distanceKm) };
      adjacency.get(hub.id).push(edge);
      edges.push(edge);
    }
  }

  return { nodes, hubs, incidents, adjacency, edges };
}
